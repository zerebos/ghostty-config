<script lang="ts">
    ...
    let pasteConfigStatus = $state<"idle" | "pasted">("idle");
    let copyConfigStatus = $state<"idle" | "copied">("idle");
    ...
    function maybeShowSharedConfigFromHash() {
        ...
        try {
            ...
            error(msg("Failed to read shared config from URL", "无法从 URL 读取分享配置"));
    ...
    async function loadConfig(candidate: string): Promise<boolean> {
        ...
        try {
            ...
            await showAlert({
                title: msg("Could not parse config", "无法解析配置"),
                message: msg("Something went wrong while parsing your config. Please open an issue on GitHub.", "解析配置时出错，请在 GitHub 上提交 issue。"),
        ...
        try {
            ...
            await showAlert({
                title: msg("Could not load config", "无法加载配置"),
                message: msg("Something went wrong while loading your parsed config. Please open an issue on GitHub.", "加载解析后的配置时出错，请在 GitHub 上提交 issue。"),
                buttonText: t("Dismiss")
            });
            return false;
        }
    ...
    async function pasteConfig() {
        if (pasteConfigStatus === "pasted") return;

        try {
            const text = await window.navigator.clipboard.readText();
            pasteConfigStatus = "pasted";
            setTimeout(() => (pasteConfigStatus = "idle"), LABEL_RESET_TIMEOUT_MS);
            const loaded = await loadConfig(text);
            if (loaded) success(msg("Config loaded from clipboard", "已从剪贴板加载配置"));
        }
        catch {
            error(msg("Clipboard access failed! Please paste manually or import from file.", "剪贴板访问失败！请手动粘贴或从文件导入。"));
        }
    }
    ...
    function selectFile() {
        ...
        reader.addEventListener("load", (event) => {
            ...
            void loadConfig(loadedText).then((didLoad) => {
                if (didLoad) success(msg("Config loaded from file", "已从文件加载配置"));
            });
        });
    ...
    function stringifyConfig(includeHeader = true) {
        ...
        const lines = includeHeader ? [`# ${msg("Config generated by Ghostty Config", "由 Ghostty 配置生成器生成的配置")}\n`] : [];
    ...
    async function copyConfig() {
        ...
        if (copyConfigStatus === "copied") return;

        try {
            await window.navigator.clipboard.writeText(stringifyConfig());
            copyConfigStatus = "copied";
            success(msg("Config copied to clipboard", "配置已复制到剪贴板"));
            setTimeout(() => (copyConfigStatus = "idle"), LABEL_RESET_TIMEOUT_MS);
        }
        catch {
            error(msg("Clipboard access failed! Please copy manually or export to file.", "剪贴板访问失败！请手动复制或导出到文件。"));
        }
    }
    ...
    async function importSharedConfig() {
        if (sharedConfigPreview) {
            ...
            if (loaded) success(msg("Shared config imported", "已导入分享配置"));
        }
    ...
    function downloadConfig() {
        ...
        success(msg("Config file downloaded", "配置文件已下载"));
    }
...
<Page title={t("Import & Export")}>
    <Group flex={1}>
        <div class="preview">
            {#if hasExportableConfig}
                <div class="row p2"># {msg("Config generated by Ghostty Config", "由 Ghostty 配置生成器生成的配置")}</div>
            {:else}
                <div class="row p2"># {msg("No changes to the default config yet", "尚未修改默认配置")}</div>
            {/if}
            <div class="row">&nbsp;</div>
        ...
        <Item name={t("Import")}>
            <div class="button-group">
                <Button onclick={pasteConfig} title={t("Paste")}>{pasteConfigStatus === "pasted" ? msg("Pasted!", "已粘贴！") : t("Clipboard")}</Button>
                <input id="config-input" type="file" onchange={selectFile} bind:this={filePicker} />
                <Button onclick={openFilePicker} title={t("Upload")}>{t("File...")}</Button>
            </div>
        </Item>
        <Separator />
        <Item name={t("Export")}>
            <div class="button-group">
                <Button
                    onclick={copyConfig}
                    title={hasExportableConfig ? t("Copy") : t("No changes yet!")}
                    disabled={!hasExportableConfig}
                >{copyConfigStatus === "copied" ? msg("Copied!", "已复制！") : t("Clipboard")}</Button>
                <Button
                    onclick={downloadConfig}
                    title={hasExportableConfig ? t("Download") : t("No changes yet!")}
                    disabled={!hasExportableConfig}
                >{t("File...")}</Button>
                <Button
                    primary
                    onclick={openShareComposer}
                    title={hasExportableConfig ? t("Share your config") : t("No changes yet!")}
                    disabled={!hasExportableConfig}
                >{t("Share...")}</Button>
            </div>
        </Item>
