<script lang="ts">
        ...
        for (const char of data) {
            ...
            if (char === "\r" || char === "\n") {
                ...
                if (result.clear) {
                    ...
                    for (const line of result.lines) term.write(line + "\n");
                    if (result.lines.length > 0) term.write("\n");
                }
                writePrompt();
            }
            else if (code === 127) {
    ...
    async function recreateTerminal() {
        ...
        term = new WTerm(container, {
            onData: shell.handleInput.bind(shell),
            autoResize: true,
        });
        ...
        term.write(bold("Welcome to the interactive terminal preview!"));
        term.write(`Type ${bold("help")} to see available commands.\n`);
        writePrompt();
    }
...
{:else}
    <Group title="Interactive Terminal" note="Try it out! Type commands to interact with a simulated shell. Supported: ls, cd, pwd, cat, echo, clear, whoami, hostname, date, uname, mkdir, touch, help">
        <div class="terminal-wrapper">
            <div class="terminal-placeholder" bind:this={container}>
            </div>
        </div>
    </Group>
{/if}
