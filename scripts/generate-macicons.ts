 ...
 *
 * Output: PNGs copied into STATIC_PATH and a regenerated macicons.ts
...
const STATIC_PATH = join("static", "macicons");
const OUTPUT_DIR = join(REPO_ROOT, STATIC_PATH);
...
export function renderModule(entries: AssetEntry[], localFileByKey: Record<string, string>): string {
    ...
    lines.push("", "");
    lines.push(`export const iconBase = "/${STATIC_PATH.replace(/^static\//, "")}";`);
    lines.push("");
 ...
 * separate manifest file. After the PNGs are copied into STATIC_PATH,
...
function buildGitDiffSummary(): string {
    ...
    const modified = run(`git diff --name-only -- ${STATIC_PATH}`).split("\n").filter(Boolean).map(f => basename(f));

    // Files not yet in the index at all (newly added this run)
    const added = run(`git ls-files --others --exclude-standard -- ${STATIC_PATH}`).split("\n").filter(Boolean).map(f => basename(f));

    // Files git tracked that are now missing from the working tree (deleted)
    const deleted = run(`git ls-files --deleted -- ${STATIC_PATH}`).split("\n").filter(Boolean).map(f => basename(f));
